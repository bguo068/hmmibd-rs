...
// Can filter out sample pairs with high discordance (based on sites where at
// least
//  1 of the genomes has the minor allele) to skip pairs with little or no
//  sharing
// Handles both within and across population comparisons. There are always two
// populations in
//  the code; in case of single pop, the 2nd pop just points to the first.
//  (iflag2 == 1) means two pops.
// States: 0=IBD (identical by descent), 1=non-identical
//  HMM notation follows Rabiner (Proc of the IEEE, V77, p257 (1989)).
//  Note: alpha/beta underflow can be handled either by working in log space +
//  logsumexp trick, or
...
int main(int argc, char **argv)
{
  ...
  char out_filebase[128], freq_file1[128], freq_file2[128], good_file[128],
      bad_file[128];
  int max_bad = 100;
  int max_good = 200;
  int linesize = 4000;
  ...
  int itoken, nsample1 = 0, nsample2 = 0, isamp, chr, sum, iall, all, js,
              snp_ind;
  int **geno1, **geno2, chr2, pos2, majall, npair_report;
  double **discord, pright, seq_ibd_fb = 0, seq_dbd_fb = 0, p_ibd, fmean,
                            fmeani, fmeanj;
  double **freq1 = NULL, **freq2 = NULL, *ffreq1 = NULL, *ffreq2 = NULL, xisum,
         xi[2][2], trans_pred, trans_obs;
  double *phi[2], pinit[2], pi[2], *b[2], a[2][2], ptrans, *alpha[2], *beta[2],
      *scale;
  double maxval, max_phi = 0, max_phiL, seq_ibd, seq_dbd, count_ibd_fb,
                 count_dbd_fb;
  double gamma[2], last_pi = 0, last_prob = 0, last_krec = 0, delpi, delprob,
                   delk, maxfreq;
  FILE *inf1 = NULL, *inf2 = NULL, *outf = NULL, *pf = NULL, *ff1 = NULL,
       *ff2 = NULL;
  int *diff = NULL, *same_min = NULL, jsamp, *allcount1, *allcount2,
      *use_sample1, *use_sample2;
  int *traj, add_seq, nsample_use2;
  int nsample_use1, nsnp, ipair, npair, isnp, chrlen, *pos, *psi[2], max, iline;
  int *nmiss_bypair = NULL, totall1, totall2, *start_chr = NULL,
      *end_chr = NULL, is, maxlen;
  int **use_pair = NULL, *nall = NULL, killit, nuse_pair = 0, gi, gj, delpos;
  int ntri = 0, ibad, nbad, start_snp, ex_all = 0, last_snp, c, iflag1, iflag2,
      oflag;
  int freq_flag1, freq_flag2, fpos = 0, fchr = 0, iter, ntrans, mflag,
                              finish_fit, bflag, gflag;
  ...
  strcpy(usage_string, "Usage: hmm -i <input file, pop1> -o <output filename> "
                       "[-I <input file, pop2>] \n");
  strcat(usage_string, "[-m <max fit iter>] [-f <allele freq file, pop1>] [-F "
                       "<allele freq file, pop2>]\n");
  strcat(usage_string,
  ...
  fprintf(pf, "sample1\tsample2\tN_informative_sites\tdiscordance\tlog_p\tN_"
              "fit_iteration\tN_generation");
  fprintf(pf, "\tN_state_transition\tseq_shared_best_traj\tfract_sites_"
              "IBD\tfract_vit_sites_IBD\n");
  ...
  // Note: using newLine1 for both files until we start reading genotypes. This
  // way newLine2 only
  ...
  fprintf(stdout,
          "sample pairs analyzed (filtered for discordance and informative "
          "markers): %d\n",
          nuse_pair);
          ...
          for (chr = 1; chr <= nchrom; chr++)
          {
            ...
            for (isnp = start_chr[chr]; isnp <= end_chr[chr]; isnp++)
            {
              ...
              else if (gi == gj)
              {
                ...
                b[0][snp_ind] =
                    pright * pright * fmean + eps * eps * (1 - fmean); // IBD
                b[1][snp_ind] =
                    pright * pright * freq1[isnp][gi] * freq2[isnp][gj] +
              ...
              else
              {
                ...
                { // index over state of current snp
                  ...
                  { // index over state of previous snp
                    ...
                    alpha[js][snp_ind] +=
                        alpha[is][snp_ind - 1] * a[is][js] * b[js][snp_ind];
                  }
